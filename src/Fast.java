...
public class Fast {
    ...
    public static void main(String[] args) throws InterruptedException {
        PointSlope p = new PointSlope();
        ArrayList<PointSlope> combined = new ArrayList<>();
        JFrame frame;
        Scanner input = null;
        File f;
	...
	// Sorting points by natural order (lexicographic order). Makes finding end points of line segments easy.
        ...
        for (int i = 0; i < count; i++){
            ...
            sortedList.add(points.get(i).slopeTo(points.get(j)));
            p.setP(points.get(j));
            p.setAngle(points.get(i).slopeTo(points.get(j)));
            combined.add(p);
            sortedList.add(p.getAngle());
                System.out.println(points.get(i).slopeTo(points.get(j)));
            }
            Collections.sort(p.getAngle(), new SlopeCompare());
        }
    ...
    private static class SlopeCompare implements Comparator<Double> {
